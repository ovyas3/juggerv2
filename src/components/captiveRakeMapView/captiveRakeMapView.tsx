...
export default function CaptiveRakeMapView() {
  ...
  }, [activeFilter, coordsData, loadedRakes, emptyRakes]);

  const handleFilterClick = (filter: 'total' | 'loaded' | 'empty') => {
    setActiveFilter(activeFilter === filter ? 'total' : filter);
  };
  ...
  async function getCoords() {
    try {
      ...
      if (response?.statusCode === 200 && Array.isArray(response?.data.data)) {
        ...
        const empty = coords.filter((val: any) => val.loading_status !== "L");
  ...
  const getIcon = (status: string) => {
    // Switch to train icon when zoomed in (zoom level > 9)
    if (currentZoom > 9) {
      return createTrainIcon(status === "L");
    }
    // Use dot markers for zoomed out view
        ...
        >
          ...
          <div className={styles.rightPanel}>
            <MapContainer
              ...
              center={center}
              zoom={5}
              style={{ height: "100%", width: "100%", padding: "0px", zIndex: "0" }}
