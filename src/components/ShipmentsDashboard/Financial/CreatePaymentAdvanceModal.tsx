      ...
      try {
        ...
        if (shipmentRes.statusCode === 200) {
          ...
          const temp: ShipmentItem = {
            ...
            total_amount:
              shipmentData.estimated?.price ||
              shipmentData.estimated?.price_per_weight ||
              0,
            advance: shipmentData.advance_amount || 0,
            remaining_actual:
              (shipmentData.estimated?.price || 0) -
              (shipmentData.advance_amount || 0),
            remaining:
              (shipmentData.estimated?.price || 0) -
              (shipmentData.advance_amount || 0),
            paying_now: 0,
    ...
    try {
      ...
      const payload = {
        ...
        payer_info: {
          ...
          ...(paymentType === "UPI" && {
            transaction_ID: request.payer_info.transaction_ID,
          }),
          ...(paymentType === "cheque" && {
      ...
      const response = await httpsPost(
        "carrier_bill_advice_shipment/create",
        payload
      );
  ...
  const calculatePayNow = (index: number) => {
    ...
    if (currentShipment.paying_now > currentShipment.remaining_actual) {
      ...
      const tds = parseFloat(
        (currentShipment.paying_now * (tdsPercent / 100)).toFixed(2)
      );
      const payingOut = parseFloat(
        (currentShipment.paying_now - tds).toFixed(2)
      );
      const remaining = parseFloat(
        (
          currentShipment.total_amount -
          currentShipment.advance -
          currentShipment.paying_now
        ).toFixed(2)
      );
  ...
  return (
    <div className={styles.modalOverlay} onClick={onClose}>
      <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
        <ModalHeader
          title={`Create Payment Advice - #${data?.shipment?.sin}`}
          onClose={onClose}
        />
        ...
        {(paymentType === "cheque" || paymentType === "bank") && (
          <div className={styles.bankDetails}>
            ...
            <div className={styles.accountDetails}>
              <div className={styles.item}>Bank Name: {bankDetails.name}</div>
              <div className={styles.item}>
                Account Number: {bankDetails.account_number}
              </div>
              <div className={styles.item}>IFSC: {bankDetails.ifsc_code}</div>
            </div>
          </div>
        )}
            ...
            <tbody>
              {shipmentList.map((item, index) => (
                <tr key={item._id}>
                  <td>{item.SIN}</td>
                  <td>
                    {item.currency_symbol} {item.advance}
                  </td>
                  <td>
                    {item.currency_symbol} {item.total_amount}
                  </td>
                  <td>
                    {item.currency_symbol} {item.remaining}
                  </td>
                  <td>
                    <input
                      type="number"
                  ...
                  <td>
                    {item.currency_symbol} {item.tds}
                  </td>
                  <td>
                    {item.currency_symbol} {item.paying_out}
                  </td>
                  <td>
