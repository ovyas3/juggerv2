      ...
      <div className={styles.modalContent} onClick={e => e.stopPropagation()}>
        ...
        <div className={styles.modalBody}>
          <div className={styles.sinNumber}>
            <p>SIN: {orderSIN || 'Not available'}</p>
          </div>
                ...
                {pickupAddresses.length > 0 ? (
                  pickupAddresses.map((address, index) => (
                    <div key={`pickup-${index}`} className={styles.addressItem}>
                      <div className={styles.addressBadge} style={{ backgroundColor: '#0b7d2e' }}>
                        P{index + 1}
                      </div>
                  ...
                  <div className={styles.noAddress}>No pickup addresses available</div>
                )}
              </div>
            </div>
              ...
              <div className={styles.addressContent}>
                {deliveryAddresses.length > 0 ? (
                  deliveryAddresses.map((address, index) => (
                    <div key={`delivery-${index}`} className={styles.addressItem}>
                      <div className={styles.addressBadge} style={{ backgroundColor: '#d8511f' }}>
                        D{index + 1}
                      </div>
                      <div className={styles.addressText}>{address}</div>
                    </div>
                  ...
                  <div className={styles.noAddress}>No delivery addresses available</div>
                )}
              </div>
            </div>
          </div>
