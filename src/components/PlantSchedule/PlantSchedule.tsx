...
// const StyledDatePicker = styled(DatePicker) <{ theme: typeof themes[ThemeKey] }>`
//   border-radius: 8px;
...
//     position: absolute;
//     right: 32px; /* Position it before the calendar icon */
//     top: 50%;
//     transform: translateY(-50%);
//     width: 16px;
//     height: 16px;
...
//   const [slotStartHourStr, slotStartMinuteStr] = timeSlot.split(':');
//   const slotStartHour = parseInt(slotStartHourStr, 10);
//   const slotStartMinute = parseInt(slotStartMinuteStr, 10);
//     const nextSlotHour = (slotStartHour + 1) % 24;

//  const nextSlotStartTime = dayjs().hour(nextSlotHour).minute(0);
...
//                return currentTime.isBefore(nextSlotStartTime);
//           }
//     }
//      if(currentHour === nextSlotHour){
//            const currentTime = dayjs();
//                 return currentTime.isBefore(nextSlotStartTime);
//         }
...
// const StyledRow = styled.tr<{ theme: typeof themes[ThemeKey];  }>`
//   background: ${props => `${props.theme.cardBg}`};
//   border-top: 2px solid ${props => props.theme.hover};
//   border-bottom: ${props => props.isTargetRow ? `1px solid ${props.theme.hover}` : 'none'};
//    background: ${props => props.isCurrentTime ? `${props.theme.accent}25` : 'transparent'};
...
//       const timeStr = item.timeSlot.split(':')[0];
...
//     fetchTargetData(selectedDate); // Fetch target data on date change
//   }, [selectedDate]);
...
//   const fetchTargetData = async (date: Dayjs) => {
//     try {
//       const response = await httpsGet(
//         `/invoice/mills_target?from=${date.format('YYYY-MM-DD')}`,
//         0,
//         router
//       );

//       if (response.statusCode === 200) {
//         const targetResponse = response as TargetResponse; // Type assertion
//         const targetsMap: { [plantName: string]: number } = {};
//         targetResponse.data.plants.forEach((plant) => {
//           targetsMap[plant.name] = plant.target;
//         });
//         console.log('targetResponse',targetsMap);
//         setPlantTargets(targetsMap);
//       } else {
//         console.error('Error fetching target data:', response);
//       }
//     } catch (error) {
...
//                     <thead>
//                       <tr>
//                         <Th isTimeColumn theme={themes[currentTheme]}>Time Slot</Th>
//                         {plants.map((plant) => (
...
//                           );
//                         })}
//                         <StyledCell theme={themes[currentTheme]} />
//                       </StyledRow>
...
// const StyledRow = styled.tr<{ theme: typeof themes[ThemeKey]; }>`
//   background: ${props => `${props.theme.cardBg}`};
//   border-top: 1px solid ${props => props.theme.hover}; /* Reduced border */
//   border-bottom: ${props => props.isTargetRow ? `1px solid ${props.theme.hover}` : 'none'};
//    background: ${props => props.isCurrentTime ? `${props.theme.accent}25` : 'transparent'};
//     &:hover {
//     td:not([data-total="true"]) {
