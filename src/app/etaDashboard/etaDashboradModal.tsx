...
const columns: readonly Column[] = [
  ...
  { id: "lastLocation", label: "Last Location", style: "header_lastLocation" },
  ...
  return shipment.map(
    (shipment: {
      ...
      return {
        ...
        fnr: { fnr: shipment?.FNR ? shipment?.FNR : "NA" },
        destination: shipment?.delivery_location
          ? shipment?.delivery_location
          : "NA",
        rrDate: {
          date: shipment?.rr_document[0]?.rr_date
            ? service.utcToist(shipment?.rr_document[0]?.rr_date)
            : "NA",
          time: shipment?.rr_document[0]?.rr_date
            ? service.utcToistTime(shipment?.rr_document[0]?.rr_date)
            : "NA",
        },
        initialEta: {
          date: shipment?.initial_eta
            ? service.utcToist(shipment?.initial_eta)
            : "NA",
          time: shipment?.initial_eta
            ? service.utcToistTime(shipment?.initial_eta)
            : "NA",
        },
        currentEta: {
        ...
        remarks: shipment?.remarks
          ? shipment?.remarks[0]?.remark || "NA"
          : "NA",
        drawnOut: {
          date: shipment?.drawnout_time
            ? service.utcToist(shipment?.drawnout_time)
            : "NA",
          time: shipment?.drawnout_time
            ? service.utcToistTime(shipment?.drawnout_time)
            : "NA",
        },
        lastUpdated: {
          date: shipment?.fois_last_updated
            ? getDelayTime(shipment?.fois_last_updated)
            : "NA",
        },
        fois_last_location: {
          status: shipment?.fois_last_location
            ? shipment?.fois_last_location
            : "NA",
        },
      };
    }
                          ...
                          return (
                            ...
                            >
                              ...
                              {column.id === "initialEta" &&
                                (row.initialEta.date !== "NA" &&
                                row.initialEta.time !== "NA" ? (
                                  <>
                              ...
                              {column.id === "currentEta" &&
                                (row.currentEta.date !== "NA" &&
                                row.currentEta.time !== "NA" ? (
                                  <>
                              ...
                              {column.id === "drawnOut" &&
                                (row.drawnOut.date !== "NA" &&
                                row.drawnOut.time !== "NA" ? (
                                  <>
                                    <div>{row.drawnOut.date}</div>
                                    <div>{row.drawnOut.time}</div>
                                  </>
                                ) : (
                                  "NA"
                                ))}
                              {column.id === "lastUpdated" &&
                                (row.lastUpdated.date !== "NA" ? (
                                  <>
                              ...
                              {column.id === "lastLocation" && (
                                <div>{row.fois_last_location.status}</div>
                              )}
                            </TableCell>
                          );
                        })}
